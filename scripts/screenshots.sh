...
HTML
  google-chrome --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=2 \
    --screenshot="$RAW/raw-desktop-$theme.png" --window-size=1240,760 \
    --virtual-time-budget=2000 "$BASE/?theme=$theme" >/dev/null 2>&1
  google-chrome --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=2 \
    --screenshot="$RAW/raw-phone-$theme.png" --window-size=520,1040 \
    --virtual-time-budget=2000 "file://$RAW/_phone-$theme.html" >/dev/null 2>&1
done
python3 - "$OUT" "$RAW" <<'PY'
import sys
...
for theme in ("dark", "light"):
    for kind in ("desktop", "phone"):
        im = Image.open(f"{raw}/raw-{kind}-{theme}.png").convert("RGB")
        bg = im.getpixel((0, 0))
        ...
        if kind == "phone":
            im = im.crop((0, 0, im.width, min(im.height, 1500)))  # drop the empty iframe tail
        im.save(f"{out}/{kind}-{theme}.png")
